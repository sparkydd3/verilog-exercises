...
/*
 *	codec: 7=bit headphone volume contro; 80 levels in 1 dB step
...
void set_volume(alt_u32 audio_base, int vol)
{
	...
	audio_i2c_wr_cmd(audio_base, 2, cmd);
	while (!audio_i2c_is_idle(audio_base));
	audio_i2c_wr_cmd(audio_base, 3, cmd);
}
		...
		switch(sw) {
			...
			case 1:	// record line input for 5 sec
				...
				for (i = 0; i < BUF_SIZE; i++) {
					while (audio_adc_fifo_empty(AUDIO_BASE));
			...
			case 4:	// play buffered audio data repeatedly until a key pressed
				while (!btn_is_pressed(BTN_BASE)) {
					for (i = 0; i < BUF_SIZE; i++) {
						while (audio_dac_fifo_full(AUDIO_BASE));
