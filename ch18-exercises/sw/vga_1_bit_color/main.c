...
0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 
...
void plot_color_chart(alt_u32 vga_base)
{
	...
	for (x = 0; x < DISP_GRF_X_MAX; x++) {
		for (y = 0; y < DISP_GRF_Y_MAX; y++) {
			...
			vga_wr_pix(vga_base, x, y, color_bw);
		}
	}
}
...
void plot_random_line(alt_u32 vga_base)
{
	...
	vga_plot_line(vga_base, 10, 10, 10, 10, 0x01);

	/* a vertical line */
	vga_plot_line(vga_base, 600, 0, 600, DISP_GRF_Y_MAX - 1, 0x01);

	/* a horizontal line */
	vga_plot_line(vga_base, 0, 400, DISP_GRF_X_MAX - 1, 400, 0x01);
...
void plot_text(alt_u32 vga_base) {
	...
	vga_wr_bit_ch(vga_base, 0, 0, 'a', 0x01, 1);
	vga_wr_bit_ch(vga_base, DISP_GRF_X_MAX - 8, 0, 'b', 0x01, 1);
	vga_wr_bit_ch(vga_base, 0, DISP_GRF_Y_MAX - 16, 'c', 0x01, 1);
	vga_wr_bit_ch(vga_base, DISP_GRF_X_MAX - 8, DISP_GRF_Y_MAX - 16,
		'd', 0x01, 1);
	
	vga_wr_bit_str(vga_base, 30 * 8, 3 * 16, "Hello World", 0x01, 1);
	vga_wr_bit_str(vga_base, 28 * 8, 5 * 16, "Hello World", 0x01, 2);
	vga_wr_bit_str(vga_base, 23 * 8, 8 * 16, "Hello World", 0x01, 3);

	vga_wr_bit_str(vga_base, 25 * 8, 16 * 16, msg_box, 0x01, 1);
